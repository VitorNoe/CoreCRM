...
class ClientManager {
    ...
    public static function createClient($data) {
        ...
        $validationErrors = self::validateClient($data);
    ...
    public static function updateClient($id, $data) {
        ...
        $data[\'id\'] = $id; // Adiciona o ID para validação de email duplicado
        $validationErrors = self::validateClient($data);
        if (!empty($validationErrors)) {
            return [\'errors\' => $validationErrors];
        }
    ...
    public static function validateClient($data) {
        ...
        // Validar formato do telefone (opcional, mas bom ter)
        if (!empty($data[\'phone\']) && !preg_match(\'/^[0-9()\s-]{7,20}$/\', $data[\'phone\'])) {
            $errors[] = \'Formato de telefone inválido\';
        }

        // Validar status
        $allowedStatuses = [\'active\', \'inactive\', \'prospect\', \'lead\'];
        if (!empty($data[\'status\']) && !in_array($data[\'status\'], $allowedStatuses)) {
            $errors[] = \'Status inválido. Valores permitidos: active, inactive, prospect, lead.\';
        }

        // Verificar se email já existe (melhorado)
        global $db;
        ...
        if ($existingClient) {
            if (!isset($data[\'id\']) || $existingClient[\'id\'] != $data[\'id\']) {
